...
// senseaudio-video 专用副本：入站是火山方舟 Seedance 原生格式，本插件无 host 协议、只有 native 路由。
// 相对模板只改三处：CONFIG 三行、默认 --entry native、便利拼装改成 content 数组。
//   LIVE_KEY='sk-...' node senseaudio-video.live.mjs --dry-run --prompt "..."
//   LIVE_KEY='sk-...' node senseaudio-video.live.mjs --prompt "..." --download out.mp4
//   LIVE_KEY='sk-...' node senseaudio-video.live.mjs --raw '{"model":"doubao-seedance-2-0-260128","content":[{"type":"text","text":"..."}],"ratio":"16:9","duration":5,"resolution":"720p"}'

const PLUGIN_PATH = "../../plugins/tasks/senseaudio-video/1.0.0/plugin.js";
const DEFAULT_BASE = "https://api.senseaudio.cn";
...
if (args.raw) {
  ...
  // 本插件入站是火山方舟 Seedance 原生格式：文本走 content[].type=text，
  // 图片走 content[].type=image_url（嵌套 image_url.url）+ role。
  inbound = {
    model: args.model,
  ...
  for (const url of args.images) {
    inbound.content.push({ type: "image_url", image_url: { url: url }, role: "first_frame" });
  }
}
...
// native 渲染（客户端真正看到的火山原生形状）
if (plugin.native && plugin.native.taskCreated) {
  console.log("native.taskCreated=" + JSON.stringify(plugin.native.taskCreated({}, { task_id: taskId, data: submitted.taskData })));
}
if (plugin.native && plugin.native.taskStatus) {
  console.log("native.taskStatus=" + JSON.stringify(plugin.native.taskStatus({}, task)).slice(0, 400));
}
