export const meta = {
  ...
  routes: [
    ...
    // 1.0.2：删掉曾在此处的 /kling/v1/videos/* 旧版风格入口（3 条 submit + 3 条 query）。
    // 它们声明的 decode（legacyTextVideo / legacyImageVideo / legacyOmniVideo）从未导出过，
    // 而宿主约定 decode/render 只能引用 native 里的可调用成员 —— 这 6 条路由是死的。
    // 删而不是补实现，是因为 README 正文写了「不提供无模型后缀的入口」、changelog 写了
    // 「去掉无后缀入口与旧版简称路径」、路由表也只列 11 条，六处文档口径一致。
    // 注意：旧版「输入形状」的支持（image_list / element_list / mode / sound / multi_prompt）
    // 完整保留在 decodeNative 的 isLegacyShaped 分支里，模型后缀入口照常收旧形状 body。
  ],
};
