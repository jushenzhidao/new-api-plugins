export const meta = {
  ...
  routes: [
    ...
    { method: "GET", path: "/kling/v1/tasks/:task_id", type: "query", taskIdParam: "task_id", render: "queryVideo" },
  ],
};
...
function outputURL(data) {
  const candidates = [data.video_url, data.url, data.videoUrl, data.output_url];
...
export function parseTaskResult(_ctx, body) {
  ...
  if (["succeed", "success", "succeeded", "completed"].includes(raw)) return { code: 0, status: "SUCCESS", progress: "100%", url: outputURL(data) };
...
export const native = {
  ...
  createdVideo(_ctx, task) { return Object.assign({}, task.data || {}, { id: task.task_id }); },
  queryVideo(_ctx, task) { return { id: task.task_id, status: task.status === "SUCCESS" ? "succeeded" : task.status === "FAILURE" ? "failed" : "processing", url: task.url, error: task.fail_reason ? { message: task.fail_reason } : undefined }; },
};
