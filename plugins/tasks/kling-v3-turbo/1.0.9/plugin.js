...
function inferMediaType(item) {
  const explicit = mediaType(item && item.type);
  if (text(item && item.type)) return explicit;
  const mime = text(item && (item.mime_type || item.mimeType)).toLowerCase();
  if (mime.indexOf("video/") === 0 || text(item && item.video_url)) return "video";
  if (mime.indexOf("image/") === 0 || text(item && item.image_url)) return "image";
  if (mime.indexOf("audio/") === 0 || text(item && item.audio_url)) return "audio";
  return "file";
}
...
function urlFieldType(key) {
  ...
  if (name === "image" || name.indexOf("image_url") >= 0 || name.indexOf("imageurl") >= 0) return "image";
  if (name === "video" || name.indexOf("video_url") >= 0 || name.indexOf("videourl") >= 0) return "video";
  if (name === "audio" || name.indexOf("audio_url") >= 0 || name.indexOf("audiourl") >= 0) return "audio";
  if (name === "file" || name.indexOf("file_url") >= 0 || name.indexOf("fileurl") >= 0) return "file";
  return "";
}
...
function scalarArtifact(item, hint) {
  ...
  const type = mediaType(item.type || urlFieldType(hint) || (item.video_url || item.video ? "video" : item.image_url || item.image ? "image" : item.audio_url || item.audio ? "audio" : item.file_url || item.file ? "file" : ""));
  const url = validURL(item.url || item.content_url || item.image_url || item.video_url || item.file_url || item.audio_url || item.image || item.video || item.file || item.audio);
  return url ? { key: text(item.key), type: type, mimeType: text(item.mime_type || item.mimeType), url: url } : null;
}
...
function artifactCandidates(data) {
  ...
  function walk(value, hint, depth) {
    ...
    const isContainer = value && typeof value === "object" && !Array.isArray(value) && !text(value.type) && !text(hint) && !validURL(value.url || value.content_url || value.image_url || value.video_url || value.file_url || value.audio_url || value.image || value.video || value.file || value.audio);
    ...
    if (Array.isArray(value)) {
      value.forEach(function (item) { walk(item, hint, depth + 1); });
      return;
    }
    Object.keys(value).forEach(function (key) {
      ...
      else if (keyType && child && typeof child === "object") walk(child, key, depth + 1);
      else if (key === "outputs" || key === "output" || key === "videos" || key === "images" || key === "audios" || key === "files" || key === "artifacts" || key === "data" || key === "result") walk(child, hint, depth + 1);
    });
  }
  walk(root, "", 0);
  return found;
...
function findArtifact(data, key) {
  ...
  return artifactList(data).find(function (item) { return item.key.toLowerCase() === wanted || item.type === wanted; }) || null;
}

function outputURL(data) {
  const root = data && typeof data === "object" ? data : {};
  const direct = validURL(root.video_url || root.url || root.videoUrl || root.output_url);
  if (direct) return direct;
  const video = findArtifact(root, "video");
...
export function listArtifacts(task) {
  ...
  return artifactList(data).map(function (item) {
    return {
      key: item.key,
      type: item.type,
      mimeType: item.mimeType || (item.type === "video" ? "video/mp4" : item.type === "image" ? "image/*" : item.type === "audio" ? "audio/*" : "application/octet-stream"),
    };
  });
}

export function buildContentRequest(ctx) {
  const key = text(ctx && ctx.artifactKey).toLowerCase();
  const data = ctx && ctx.data && typeof ctx.data === "object" ? ctx.data : {};
  const artifact = findArtifact(data, key);
  const url = validURL(ctx && ctx.url) || (artifact && artifact.url) || (key === "video" ? outputURL(data) : "");
...
export const native = {
  ...
  queryVideo(_ctx, task) {
    const data = task.data && typeof task.data === "object" ? task.data : {};
    const url = task.url || outputURL(data) || outputURL(data.data && data.data[0]);
    return Object.assign({}, data, { id: task.task_id || data.id, task_id: task.task_id || data.task_id, status: task.status === "SUCCESS" ? "succeeded" : task.status === "FAILURE" ? "failed" : "processing", url: url });
  },
};
