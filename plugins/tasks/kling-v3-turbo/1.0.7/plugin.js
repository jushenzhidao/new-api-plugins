...
export function buildContentRequest(ctx) {
  ...
  const url = ctx.url || outputURL(data) || outputURL(data.data && data.data[0]);
...
export const native = {
  ...
  queryVideo(_ctx, task) {
    ...
    return { id: task.task_id || data.id, status: task.status === "SUCCESS" ? "succeeded" : task.status === "FAILURE" ? "failed" : "processing", url: url, error: task.fail_reason ? { message: task.fail_reason } : undefined };
  },
};
