...
export function parseTaskResult(ctx, body) {
  ...
  if (h3Task) {
    const rawStatus = trimmed(h3Task.status).toLowerCase();
    const h3Statuses = {
      queued: "QUEUED",
      pending: "QUEUED",
      created: "QUEUED",
    ...
    const h3Status = h3Statuses[rawStatus];
    if (!h3Status) {
...
export const native = {
  ...
  createdVideo(_ctx, task) {
    return {
      id: task.publicTaskId || task.task_id,
      object: "video",
      model: H3_MODEL,
      status: "queued",
    };
  ...
  queryVideo(_ctx, task) {
    ...
    const output = {
      ...
      object: "video",
      model: H3_MODEL,
      status: statuses[rawStatus] || "unknown",
      progress: Number(String(task.progress || "0").replace("%", "")),
    };
    if (rawStatus === "SUCCESS" || rawStatus === "SUCCEEDED" || rawStatus === "COMPLETED") output.url = h3ArtifactURL(task);
    if (rawStatus === "FAILURE" || rawStatus === "FAILED") output.error = { message: task.reason || "task failed" };
    return output;
  },
};
