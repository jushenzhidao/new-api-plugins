...
export const native = {
  ...
  queryVideo(_ctx, task) {
    ...
    const output = {
      ...
      object: "video",
      model: H3_MODEL,
      status: statuses[String(task.status || "").toUpperCase()] || "unknown",
      progress: Number(String(task.progress || "0").replace("%", "")),
    };
    if (task.status === "SUCCESS") output.url = h3ArtifactURL(task);
    if (task.status === "FAILURE") output.error = { message: task.reason || "task failed" };
    return output;
  },
};
