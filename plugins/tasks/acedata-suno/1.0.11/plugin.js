export const meta = {
  ...
  // batch：宿主把同一批在途任务合成一次 buildBatchQueryRequest（action=retrieve_batch + ids），
  // 每轮只打一次上游，避免任务多时把 /suno/tasks 打爆（429）。per-task hook 仍保留做合同兼容。
  fetchMode: "batch",
  usageSchema: {
    clips: {
      type: "number",
...
//   POST /suno/tasks   { id,  action: "retrieve" }       -> 单个任务包裹体（per-task 兼容路径）
//                      { ids, action: "retrieve_batch" } -> { items: [<包裹体>...], count }
//                      包裹体：{ _id, id, created_at, started_at, finished_at?, elapsed?,
//                                request:{action,prompt}, response:{success, task_id, data[]} }
...
// ---------------------------------------------------------------------------
// 批量查询（fetchMode: "batch"）
//   POST /suno/tasks { action: "retrieve_batch", ids: string[] }
//   -> { items: [<单个任务包裹体>, ...], count: n }
// 每个 item 与单查包裹体同形，故状态判定、制品提取和渲染完全复用同一套逻辑。
// 结果必须按 items[].id 回填 taskId：上游不保证返回顺序与请求 ids 一致。
// ---------------------------------------------------------------------------

export function buildBatchQueryRequest(ctx, tasks) {
  const ids = [];
  for (const task of tasks || []) {
    const id = trimmed(task && task.taskId);
    if (id) ids.push(id);
  }
  return {
    url: apiBase(ctx) + "/suno/tasks",
    method: "POST",
    headers: upstreamHeaders(ctx),
    body: { action: "retrieve_batch", ids: ids },
  };
}

// 从 request 反推规范化 action：音乐任务 request.action = generate/extend/concat，歌词任务只有 prompt
function requestActionOf(item) {
  const req = item && item.request && typeof item.request === "object" && !Array.isArray(item.request) ? item.request : {};
  return trimmed(req.action) || "lyrics";
}

export function parseBatchResult(ctx, body, response) {
  let parsed = body;
  if (typeof parsed === "string") {
    try {
      parsed = JSON.parse(parsed);
  ...
  const httpCode = queryHTTPStatus(ctx, response);
  if (httpCode >= 400) {
    throw new Error(envelopeError(parsed) || "acedata tasks batch query error (http " + httpCode + ")");
  }
  // 批量端点自身的错误（token/限流/内部错误）抛错重试，不把整批判成任务失败
  const topError = envelopeError(parsed);
  if (topError) throw new Error(topError);
  // 容错：上游万一退化成单个包裹体，也按单元素处理
  const raw = Array.isArray(parsed.items) ? parsed.items : parsed && parsed.id ? [parsed] : [];
  const out = [];
  for (const item of raw) {
    if (!item || typeof item !== "object" || Array.isArray(item)) continue;
    const id = trimmed(item.id);
    if (!id) continue;
    const r = statusResult(item);
    const result = {
      taskId: id,
      action: requestActionOf(item),
      status: r.status,
      reason: r.reason || "",
      progress: r.progress,
      submitTime: epochSeconds(item.created_at),
      startTime: epochSeconds(item.started_at),
      finishTime: epochSeconds(item.finished_at),
      // 持久化整包包裹体：制品提取与 renderer 依赖 task.data.response
      data: item,
    };
    out.push(result);
  }
