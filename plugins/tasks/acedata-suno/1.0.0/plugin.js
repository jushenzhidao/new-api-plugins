export const meta = {
  ...
  routes: [
    // 跨协议异构：路径保真 New API Suno 官方契约（/{biz}=/acedata + Suno-API 原生路径），
    // body 需要 New API Suno 形状 -> acedata /suno/audios|/suno/lyrics 的结构转换，adapterMode 恒为 heterogeneous
    {
      method: "POST",
      path: "/acedata/suno/submit/music",
      type: "submit",
      models: ["suno_music"],
      decode: "submitMusic",
      render: "submitted",
    ...
    {
      method: "POST",
      path: "/acedata/suno/submit/lyrics",
      type: "submit",
      models: ["suno_lyrics"],
      decode: "submitLyrics",
      render: "submitted",
    ...
    {
      method: "GET",
      path: "/acedata/suno/fetch/:task_id",
      type: "query",
      taskIdParam: "task_id",
      render: "fetchTask",
    },
...
// ---------------------------------------------------------------------------
// 入站契约：New API Suno 任务格式（基于开源 Suno-API 代理协议）
...
//   status 枚举：SUBMITTED / IN_PROGRESS / SUCCESS / FAIL
// 上游契约：AceDataCloud Suno（https://api.acedata.cloud）
//   POST /suno/audios  { action, model, custom, prompt|lyric, title, style, instrumental,
//                        audio_id, continue_at, async, ... }  Bearer 鉴权
...
//   - POST /suno/fetch 批量查询：宿主 query 路由合同不支持无 :task_id 占位符的路径
// ---------------------------------------------------------------------------
...
function decodeSubmitMusic(ctx) {
  ...
  if (continueClipId) {
    ...
    if (prompt) normalized.lyric = prompt;
    if (trimmed(req.title)) normalized.title = trimmed(req.title);
    if (trimmed(req.tags)) normalized.style = trimmed(req.tags);
  } else if (gptPrompt) {
    // 灵感模式：gpt_description_prompt -> acedata prompt（custom=false）
    ...
    normalized.prompt = gptPrompt;
  } else {
    // 自定义模式：prompt 即歌词 -> acedata lyric（custom=true）
    if (!prompt && !instrumental) throw new Error("prompt (lyrics) or gpt_description_prompt is required");
    normalized.operation = "generate";
...
function encodeAcedataRequest(payload) {
  if (payload.kind === "lyrics") {
    // POST /suno/lyrics：{ prompt, async }
    return { prompt: payload.prompt, async: true };
  }
  // POST /suno/audios：恒 async，靠 /suno/tasks 轮询
  const body = { action: payload.operation === "extend" ? "extend" : "generate", async: true };
  ...
  if (payload.operation === "extend") {
    ...
    body.continue_at = payload.continueAt;
...
export function extractUsage(ctx) {
  ...
  return { clips: SONGS_PER_TASK, operation: payload.operation || "generate" };
}
...
export const native = {
  ...
  fetchTask(_ctx, task) {
    ...
    const status = SUNO_STATUS[task.status] || "SUBMITTED";
    const out = {
